...
sa_t *prefixDoubling(sa_t *pairs, uint32_t *keys, ss_ranges *unsorted, uint n,
    ...
    while (total > 0) {
        ...
        for (uint32_t i = 0; i < kv_size(*unsorted); ++i) {
            ...
            for (sa_t *curr = pairs + kv_A(*unsorted, i).a; curr <= pairs + kv_A(*unsorted, i).b; ++curr)
                curr->b = keys[curr->a + h];
            // TODO: qsort vs ksort
            qsort(pairs + kv_A(*unsorted, i).a, kv_A(*unsorted, i).b - kv_A(*unsorted, i).a + 1, sizeof(sa_t),
